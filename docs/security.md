...
- **Prefer a GitHub App over an OAuth App.** An OAuth App's `repo` scope reaches every repository the signed-in user can reach. A GitHub App is limited to the repositories selected when it is installed, so a compromised Baguette instance cannot touch anything you did not grant it. Set `GITHUB_APP_SLUG` to enable it — see [Using a GitHub App](../README.md#using-a-github-app-recommended).
