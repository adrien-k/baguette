...
- **Grant the GitHub App only the repositories it needs.** GitHub access is always App-based, and an App is limited to the repositories selected when it is installed — so a compromised Baguette instance cannot touch anything you did not grant it. Pick repositories individually rather than "All repositories". See [Setting up the GitHub App](../README.md#setting-up-the-github-app).
...
- **The Slack bot token is not exposed to Claude's shell environment.** It is encrypted at rest, held by the Baguette server, and only ever used to post the messages the `SlackPostMessage` tool sends on the agent's behalf. Tokens are workspace-wide, though: every session can post as any configured Slack app, so scope each app to the channels you are comfortable with. See [docs/slack.md](slack.md).
